...
- Synchronize ordinary worktree entries incrementally through immutable chunks and causal node/entry events. Never reconcile individual `.git` files; Git metadata transfers incrementally but applies as one validated transaction.
...
- Harness mutations may occur only below a validated `~/codefoldersync-v2-<peer>-runs/<run-id>` root containing the matching sentinel. Tests must never register, scan, or mutate an existing `~/Code` folder on any machine.
...
Keep the focused V2 integration suite at ten tests or fewer. Run `pnpm check` before committing.
