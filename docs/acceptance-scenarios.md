...
Successful and failed isolated runs stop and uninstall every run-qualified native service. The controller allows a bounded ten-second native-manager shutdown window before it fails with the exact machine ID. A failed run retains its private workspace and recovery evidence but leaves no launchd registration or systemd link behind.
