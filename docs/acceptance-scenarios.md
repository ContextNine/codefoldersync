...
The capacity commands are read-only preflights over the exact specification and sentinel roots. They project both preserved workspace copies, one content-addressed object store per peer plus the hub, target recovery, and a final 25 percent reserve from the accepted ignore contract. The isolated command measures the protected master and available bytes on each real machine through the pinned binary. Both mutating runners repeat their capacity gate before they copy a master.
...
The live AI acceptance scenario uses an explicit model command in a generated non-secret TypeScript repository inside the run root. The runner sends a versioned prompt on standard input, observes mutations with one monotonic controller clock, validates the final repository, and stores file paths and content only in the private cassette. Hub acceptance is recognized from the accepted workspace bytes and executable meaning, not a pre-publication namespace digest whose new node identities have not been allocated yet. Cross-platform visibility likewise compares bytes and executability while the private cassette retains the source mode for exact replay. The result records model duration, first source change, final hub acceptance, per-target convergence, and path visibility p50, p95, and maximum. The second fresh repetition replays the first repetition's private cassette without invoking the model and requires the same final digest.

Successful and failed isolated runs stop and uninstall every run-qualified native service. A failed run retains its private workspace and recovery evidence but leaves no launchd registration or systemd link behind.
