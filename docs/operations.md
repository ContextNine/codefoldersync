...
`acceptance witness`, `acceptance pseudo-fleet`, and `acceptance isolated-fleet` operate only on explicitly named fixture and sentinel roots. They do not discover or default to `~/Code`. Both fleet runners verify masters before and after fresh repetitions, preserve every run directory, and stop on insufficient capacity or a changed witness. The isolated runner also requires strict SSH between every machine pair and the exact recorded build everywhere. Cleanup remains a separate exact-path operation.
