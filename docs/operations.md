...
`acceptance witness`, `acceptance pseudo-fleet`, and `acceptance isolated-fleet` operate only on explicitly named fixture and sentinel roots. They do not discover or default to `~/Code`. Both fleet runners verify masters before and after fresh repetitions, preserve every run directory, and stop on insufficient capacity or a changed witness. The isolated runner also requires strict SSH between every machine pair and the exact recorded build everywhere. Its capacity preflight runs remotely on all three machines before any repetition and reserves both retained workspace copies, peer object stores, the hub object store, target recovery, and another 25 percent. Cleanup remains a separate exact-path operation.

The isolated runner installs native service definitions disabled below each repetition root. It activates them only after every target verifies and the acceptance-only cutover projects the normal revision. At the end it stops each service and moves its definition into run-local recovery. An interrupted run keeps the service and root evidence for inspection.
